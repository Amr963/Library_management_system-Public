                        ...
                        <span>
                            <td>
                                <form method="POST" action="{{ route('author.update', $author->id) }}">
                                    @csrf
                                    @method('PUT')
                                    <input type="text" class="form-control" name='authors_name'
                                        value="{{ old('authors_name', $author->authors_name) }}">
                                    @error('authors_name')
                        ...
                        <span>
                            <td>
                                <select class="form-control" name="gender">
                                    ...
                                    <option value="0" @if (old('gender', $author->gender) == 0) selected @endif>Male</option>
                                    <option value="1" @if (old('gender', $author->gender) == 1) selected @endif>Female</option>
                                </select>
                                @error('gender')
